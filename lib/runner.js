  ...
  this.add = function(stepName, href, stepOptions, stepRequest, opcoes, servico, rota, func){
    ...
    step.run = function(callback){
      ...
      for (let quantidadeJson = 0; (quantidadeJson < totalRequests); quantidadeJson++) {
        if (!quantidadeJson) console.log('\n@@@ gerando dados para o json');
        if (quantidadeJson && (quantidadeJson % 1000 === 0)) {
          console.log(`@@@ ${quantidadeJson} dados para as requisições gerados em memória`);
        }
      ...
      console.log(`@@@ todos os dados gerados já em memória`);
      ...
      console.log(`@@@ todos os dados gerados em memória já registrados em arquivos .json\n`);
