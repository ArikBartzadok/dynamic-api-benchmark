...
module.exports = {
  make: function(req, server, suite, suiteName, requestAgent, temporizador, callback){
    requestAgent.make(req, server, temporizador, function(err, response){
  ...
  setup: function(suiteName, server, endpoint, suite, requestAgent){
    ...
    suite.runner.add(suiteName, finalURL, suiteOptions, suiteRequest, req, function(temporizador, done){
      self.make(req, server, suite, suiteName, requestAgent, temporizador, done);
    });
  }
};
